...
class G:
    ...
    # samProDict = collections.OrderedDict()
    # sorted_samPro = []
    samProList = []
    indexList = []
...
def id_do(id):
    ...
    if b!=aInterval.get_samPro():
        ...
        # aInterval.set_samPro(b)
        # G.samProDict[interIndex] = b
        # G.sorted_samPro = sorted(G.samProDict.items(), key=lambda x: x[1], reverse=True)
    G.step += 1
    record2()
    return nodeFlag
